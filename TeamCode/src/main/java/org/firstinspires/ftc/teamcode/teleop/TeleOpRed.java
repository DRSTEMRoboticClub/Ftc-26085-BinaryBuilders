...
public class TeleOpRed extends CommandOpMode {
    ...
    // Tag-visibility streak: tracks how many seconds the LL has been seeing tag 24 continuously.
    ...
    public void initialize() {
        ...
        ShooterConfig.TRACKED_TAG_ID = 24;
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive() && !isStopRequested()) {
            try {
                ...
                // Command:  adb logcat -d | grep TELE_RED
                if (now - lastLogMs >= 500) {
                    lastLogMs = now;
                    Log.d("TELE_RED", String.format(
    ...
    private void correctLocalizerFromTag(double dist, double txDeg) {
        ...
        double estX = LocalizationConfig.RED_TAG_FIELD_X - dist * Math.cos(dirRad);
        double estY = LocalizationConfig.RED_TAG_FIELD_Y - dist * Math.sin(dirRad);
